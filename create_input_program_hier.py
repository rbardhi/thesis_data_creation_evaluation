from extract_features_hier import extract_features, extract_features_det, extract_features_noisy, extract_test_features, extract_test_features_for_action, extract_evidence_0, extract_evidence_1, extract_evidence_2

EXMP_LIM = 1

dir_path = 'declarative_biases/hier/'
...
def get_declarative_bias(constrained,twoD):
  file_name = '{}_hier_{}'.format(scenario(constrained),dim(twoD))
...
def get_declarative_bias_det(constrained,twoD):
  file_name = '{}_hier_{}_det'.format(scenario(constrained),dim(twoD))
  with open(dir_path+file_name,'r') as f:
    declarative_bias = f.read()
  return declarative_bias  
  
def get_declarative_bias_dc(constrained,twoD):
  file_name = '{}_hier_{}_dc'.format(scenario(constrained),dim(twoD))
  with open(dir_path+file_name,'r') as f:
    declarative_bias = f.read()
  return declarative_bias   
