...
class Policy:
  ...
  def new_fn(self,smth,world):
    move_obj,north_obj,left_obj,base = smth
    move_obj = world.get_object(move_obj.id)
    north_obj = world.get_object(north_obj.id) if north_obj is not None else None
    left_obj  = world.get_object(left_obj.id) if left_obj is not None else None
    score = 0
    #Take in account during search if one of the subgoals is reached
    if north_obj is not None:
      if not move_obj.y > north_obj.y:
        score += abs(move_obj.y - north_obj.y) + 1
    if left_obj is not None:
      if not move_obj.x < left_obj.x:
        score += abs(move_obj.x - left_obj.x) + 1
    penalty = 0
    '''for obj in base.objects:
      if obj.id == move_obj.id:
        continue
      penalty += abs(obj.x - world.get_object(obj.id).x)
      penalty += abs(obj.y - world.get_object(obj.id).y)'''
    return score + penalty**2
  
  def get_score_fn(self,goal,base):
    return lambda world: self.scoring_fn(base,world)
      
      #this to be changed if roll back
  def new_policy(self, world, goal):
    #moves = [[self.move_left],[self.move_right]] # 1D case
    #moves = [[self.move_left,self.move_up,self.move_down],[self.move_right,self.move_down,self.move_up]]   # 2D case
    moves = [[self.move_left,self.move_up],[self.move_right,self.move_down]]
    root = Node(world.copy(),0) #init_depth
    #root.action = goal.curr_condition.__name__
    if goal.north_obj is not None:
      root.action += ['north']
    if goal.left_obj is not None:
      root.action += ['left']
    breadthFirst = SearchTree(goal,root,moves)
    try:
      smth = (goal.move_obj,goal.north_obj,goal.left_obj,world)
      output = breadthFirst.search(self.get_score_fn(goal,smth)) #state,move,obj_id,hier
  ...
  '''def new_manhattan(self,world):
    ...
    for obj in world.objects:
      if obj.shape == 'square':
        ...
        pass
    return score'''
  '''#Scoring functions for hier  #this to be changed if roll back
  def score_rmc_lt_lmt(self,smth,world):
    move_obj,rel_to_obj,base = smth
    rightmost_circle = world.get_object(move_obj.id)
    leftmost_triangle = world.get_object(rel_to_obj.id)
    ...
    #this to be changed if roll back
  def score_rmt_lt_lms(self,smth,world):
    move_obj,rel_to_obj,base = smth
    rightmost_triangle = world.get_object(move_obj.id)
    leftmost_square = world.get_object(rel_to_obj.id)
    ...
    return normal + penalty**2
    #this to be changed if roll back
  def score_rmc_lt_lms(self,smth,world):
    move_obj,rel_to_obj,base = smth
    rightmost_circle = world.get_object(move_obj.id)
    leftmost_square = world.get_object(rel_to_obj.id)
