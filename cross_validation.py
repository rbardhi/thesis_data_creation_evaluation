...
import random
import numpy as np
from write_dc_input import write_dc_input_det, write_dc_input_noisy
from learn_rules import learn_rules_det, learn_rules_noisy, sample_values
from stats import find_statistics
from create_input_program import get_evidence_features, get_test_features, get_evidence_features_1
...
def cross_validation(noise,noise_type,noise_val,constrained, scen, seed, num_exmp,first_feat):
  ...
  for train, test in generate_bins(examples):
    ...
    if DEMO:
      ...
      print('files_for_demo/{}evidence_file'.format(scen))
    ...
    if noise:           #[0.05,0.1,0.2,0.5,0.7]
      for noise_val in [0.05,0.1,0.2,0.5,0.7]:
        if twoD:
          ...
          if constrained:
            ...
            first_feat = 'shape'
          if noise:
            scen = '2D_flat_noise_{}/{}/'.format(str(noise_val),scenario(constrained))
          else:
            scen = '2D_flat/{}/'.format(scenario(constrained))
        else:
          num_exmp = formula
          if constrained:
            first_feat = ''
          else:
            first_feat = 'atleastOne'
          if noise:
            scen = 'flat_noise_{}/{}/'.format(str(noise_val),scenario(constrained))
          else:
            scen = 'flat/{}/'.format(str(noise_val),scenario(constrained))
        if not TEST:
      ...
      if twoD:
        ...
        if noise:
          ...
          scen = 'flat/{}/'.format(scenario(constrained))
      if not TEST:
        if not os.path.isfile('results/{}results.pkl'.format(scen)):
          cross_validation(noise,noise_type,noise_val,constrained,scen,seed,num_exmp,first_feat)
      else:
