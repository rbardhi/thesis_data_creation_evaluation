...
def extract_features(id,example):
  ...
  output += example.get_action_hier(id)
...
def extract_features_noisy(id, example, noise_type, noise_val):
  ...
  output += example.get_action_hier_dc(id)
  for obj in example.next_state.objects:
...
def get_relations(id,example):
  ...
  for i,rel in enumerate(rels['left']):
    for j in rel:
      output += 'left_of({},{},{},true).\n'.format(id,i,j)
  for i,rel in enumerate(rels['north']):
    for j in rel:
...
def get_relations_dc(id,example):
  ...
  for i,rel in enumerate(rels['left']):
    for j in rel:
      output += 'left_of({},{},{})~val(true) := true.\n'.format(id,i,j)
  for i,rel in enumerate(rels['north']):
    for j in rel:
      output += 'north_of({},{},{})~val(true) := true.\n'.format(id,i,j)
  return output
...
def get_relations_ev(id,example):
  ...
  for i,rel in enumerate(rels['north']):
    for j in rel:
      output += ['north_of({},{},{})~=true'.format(id,i,j)]
  return output
